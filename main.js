...
(() => {
  ...
  let touchStartX=0, touchStartY=0, touchStartTime=0, lastTouchX=null, moved=false, holdTimer=null;
  const TAP_TIME=200, TAP_MOVE=10, HOLD_TIME=350;
  const FLICK_TIME=180, FLICK_DISTANCE=60, DASH_OFFSET=90;
  ...
  function startAutoFire(){
    state.autoFire = true;
    state.lastAutoFire = 0; // fire immediately on next update
  }
  ...
  function handleStart(e){
    ...
    lastTouchX = t.clientX; moved = false;
    // start pointing
    state.player.x = pointerToCanvasX(lastTouchX);
    // schedule long-press
    clearTimeout(holdTimer);
    holdTimer = setTimeout(()=> startAutoFire(), HOLD_TIME);
  }
  function handleMove(e){
    ...
    state.player.x = pointerToCanvasX(lastTouchX);
    if (Math.abs(t.clientX - touchStartX) > TAP_MOVE || Math.abs(t.clientY - touchStartY) > TAP_MOVE){
      moved = true;
    }
  }
  function handleEnd(e){
    ...
    // Flick dash (quick + long)
    if (dt <= FLICK_TIME && adx >= FLICK_DISTANCE && ady < 80){
      const dir = dx > 0 ? 1 : -1;
      const target = Math.max(20, Math.min(W-20, state.player.x + dir*DASH_OFFSET));
      state.dashTarget = target;
      state.dashUntil = performance.now() + 160; // dash duration ~160ms
      stopAutoFire(); // end hold if any
      lastTouchX = null;
      return;
    }

    // Tap shot (short + small move)
    if (dt <= TAP_TIME && adx < TAP_MOVE && ady < TAP_MOVE){
      shoot(performance.now());
    }

    // Long-press release -> stop autofire
    stopAutoFire();
    lastTouchX = null;
  }
  ...
  wrap.addEventListener('pointerdown', (e)=>{ state.player.x = pointerToCanvasX(e.clientX); });
  wrap.addEventListener('pointermove', (e)=>{ if (e.pressure>0 || e.buttons===1) state.player.x = pointerToCanvasX(e.clientX); });
  wrap.addEventListener('pointerup', (e)=>{ stopAutoFire(); });
  ...
  function update(dt, t){
    ...
    if (state.dashTarget != null){
      ...
      if ((dir>0 && state.player.x + step >= state.dashTarget) || (dir<0 && state.player.x + step <= state.dashTarget)){
        ...
        state.player.x = state.player.x + step;
      }
      // safety time-out
